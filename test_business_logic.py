...
def test_task_business_rules():
    ...
    try:
        ...
        try:
            ...
            asyncio.run(create_dependency_tasks())
...
def test_business_scenario_simulation():
    """测试真实业务场景模拟"""
    print("\n🧪 测试真实业务场景模拟...")
    ...
    try:
        ...
        temp_dir = tempfile.mkdtemp(prefix="business_scenario_test_")
        ...
        try:
            ...
            print("  🎯 场景1: 项目开发完整流程")
            scenario_total += 1
            # 模拟项目开发流程：使命 → 任务 → 执行 → 反思
            
            # 1. 创建项目使命
            mission = task_interface.create_task({
                "title": "Q2电商平台开发",
                "task_type": TaskType.MISSION,
                "originator_id": "project_manager",
                "priority": "high",
                "idempotency_key": "q2_mission"
            })
            
            if mission["success"]:
                mission_id = mission["task"]["task_id"]
                print(f"    ✅ 项目使命创建: {mission_id}")
                
                # 2. 创建开发子任务
                dev_task = task_interface.create_task({
                    "title": "后端API开发",
                    "task_type": TaskType.COGNITIVE_STEP,
                    "originator_id": "tech_lead",
                    "priority": "high",
                    "parent_task_id": mission_id,
                    "idempotency_key": "backend_dev"
                })
                
                if dev_task["success"]:
                    dev_task_id = dev_task["task"]["task_id"]
                    print(f"    ✅ 开发任务创建: {dev_task_id}")
                    
                    # 3. 执行开发任务
                    task_interface.update_task_status(dev_task_id, TaskStatus.IN_PROGRESS, "开始开发")
                    
                    # 模拟开发过程中的问题
                    problem_reflection = reflection_interface.generate_reflection({
                        "subject": "API开发技术难题",
                        ...
                        "context": {
                            ...
                            "impact": {
                                "affected_features": ["用户查询"],
                                "performance_degradation": "50%"
                            }
                        }
                    })
                    
                    if problem_reflection["success"]:
                        print("    ✅ 问题反思生成")
                        
                        # 4. 解决问题并完成任务
                        task_interface.update_task_status(dev_task_id, TaskStatus.DONE, "问题解决，开发完成")
                        
                        # 5. 项目成功反思
                        success_reflection = reflection_interface.generate_reflection({
                            "subject": "API开发项目成功",
                            ...
                            "context": {
                                "success": {
                                    "degree": "complete",
                                    "impact_score": 0.8,
                                    "metrics": {"performance_improvement": "60%"}
                                },
                                "success_factors": [
                                    "团队协作良好",
                                    "技术方案合理",
            ...
            # 创建主任务
            main_task = task_interface.create_task({
                "title": "系统集成主任务",
                "task_type": TaskType.MISSION,
                "originator_id": "system_architect",
                "priority": "critical",
                "idempotency_key": "system_integration"
            })
            
            if main_task["success"]:
                main_task_id = main_task["task"]["task_id"]
                ...
                for title, priority in subtask_configs:
                    subtask = task_interface.create_task({
                        "title": title,
                        "task_type": TaskType.COGNITIVE_STEP,
                        "originator_id": "tech_lead",
                        "priority": priority,
                        "parent_task_id": main_task_id,
                        "idempotency_key": f"subtask_{title.replace(' ', '_')}"
                    })
                    
                    if subtask["success"]:
                        subtasks.append(subtask["task"]["task_id"])
                        print(f"    ✅ 子任务创建: {title}")
                
                # 测试依赖关系约束
                if len(subtasks) == len(subtask_configs):
                    print("    ✅ 任务依赖关系正确")
                    scenario_passed += 1
                else:
                    print("    ❌ 任务依赖关系错误")
            ...
            reflection_configs = [
                {
                    ...
                    "context": {
                        "decision": {"factors": ["A", "B", "C"]},
                        "outcome": {"success": True},
                        "insights": ["因素A重要", "因素B关键"],
                        "lessons": ["需要更多调研"],
                        "improvements": ["建立评估框架"]
                    }
                },
                {
                    "type": ReflectionType.PROCESS_REFLECTION,
                    "quality_expectation": "medium",
                    "context": {
                        "process": {"phases": ["步骤1", "步骤2"]},
                        "insights": ["流程需要优化"],
                        "lessons": ["步骤2效率低"],
                        "improvements": ["优化流程"]
                    }
            ...
            for config in reflection_configs:
                reflection = reflection_interface.generate_reflection({
                    "subject": f"{config['type'].value}质量测试",
                    "reflection_type": config["type"],
                    "context": config["context"]
                })
                
                if reflection["success"]:
                    ref_data = reflection["reflection"]
                    expected_quality = config["quality_expectation"]
                    
                    # 验证质量评估是否符合预期
                    if expected_quality == "high" and ref_data["quality"] in ["excellent", "good"]:
                        print(f"    ✅ 高质量内容评估正确: {ref_data['quality'].value}")
                        reflections.append(reflection["reflection"]["reflection_id"])
                    elif expected_quality == "medium" and ref_data["quality"] in ["good", "fair"]:
                        print(f"    ✅ 中等质量内容评估正确: {ref_data['quality'].value}")
                        reflections.append(reflection["reflection"]["reflection_id"])
                    elif expected_quality == "low" and ref_data["quality"] in ["fair", "poor"]:
                        print(f"    ✅ 低质量内容评估正确: {ref_data['quality'].value}")
                        reflections.append(reflection["reflection"]["reflection_id"])
                    else:
                        print(f"    ❌ 质量评估不符合预期: 期望{expected_quality}, 实际{ref_data['quality'].value}")
                else:
                    print(f"    ❌ 反思生成失败: {config['type'].value}")
            
            # 测试治理流程：验证高质量反思，归档低质量反思
            if len(reflections) >= 2:
                high_quality_reflection = reflections[0]
                low_quality_reflection = reflections[1]
                
                # 验证高质量反思
                verify_result = reflection_interface.verify_reflection(high_quality_reflection, "quality_expert")
                ...
                # 归档低质量反思
                archive_result = reflection_interface.archive_reflection(low_quality_reflection)
            ...
            print(f"  📊 业务场景模拟测试结果: {scenario_passed}/{scenario_total} 通过")
            return scenario_passed >= scenario_total * 0.6  # 60%通过率
            
        finally:
            shutil.rmtree(temp_dir, ignore_errors=True)
        
    except Exception as e:
        print(f"❌ 业务场景模拟测试失败: {e}")
        return False
...
def main():
    ...
    # 业务场景模拟
    results.append(test_business_scenario_simulation())
