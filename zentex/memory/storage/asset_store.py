...
class AssetDatabaseStore:
    ...
    def list_plugins(self, category: Optional[Literal["cognitive", "functional"]] = None) -> List[Dict[str, Any]]:
        with self._lock:
            ...
            return [self._decode_plugin_row(dict(row)) for row in cursor]

    def get_plugin(self, plugin_id: str) -> Optional[Dict[str, Any]]:
        with self._lock:
            cursor = self._conn.execute("SELECT * FROM system_plugins WHERE plugin_id = ?", (plugin_id,))
            row = cursor.fetchone()
            return self._decode_plugin_row(dict(row)) if row else None

    def _decode_plugin_row(self, row: Dict[str, Any]) -> Dict[str, Any]:
        spec_json = row.get("spec_json")
        if isinstance(spec_json, str):
            try:
                row["spec_json"] = json.loads(spec_json)
            except json.JSONDecodeError:
                logger.warning("Failed to decode plugin spec_json for %s", row.get("plugin_id"))
        return row
