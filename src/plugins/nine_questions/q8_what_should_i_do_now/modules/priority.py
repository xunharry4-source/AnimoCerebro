...
def derive_priority_baseline(
    ...
    q7_constraints = coerce_string_list(q7.get("non_bypassable_constraints"))
    q7_statuses = coerce_string_list(q7.get("possibility_statuses"))
    q7_ready_candidates = [
        normalize_text(item.get("possibility_description"))
        for item in (q7.get("ready_for_q4_objective_candidates") if isinstance(q7.get("ready_for_q4_objective_candidates"), list) else [])
        if isinstance(item, dict)
        if normalize_text(item.get("possibility_description"))
    ]

    if q7_ready_candidates:
        immediate_tasks.extend([f"evaluate Q7 external candidate intent: {item}" for item in q7_ready_candidates])
    else:
        immediate_tasks.append("derive Q8 intent from Q7 external public output before execution")

    if resource_gaps:
        blocked_tasks.extend([f"resolve resource gap: {item}" for item in resource_gaps])
    escalation_conditions.extend([f"Q7 external boundary constraint detected: {item}" for item in q7_constraints])
    escalation_conditions.extend([f"Q7 external possibility status requires caution: {item}" for item in q7_statuses])
