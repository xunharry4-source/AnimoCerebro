...
- Q8 must read authoritative Q1/Q2/Q3/Q7 public outputs through the upstream service accessors before synthesis.
...
2. **官方加载器路径 (Official Loader Methods)**:
   - 必须通过上游插件提供的对外读取方法从 SQLite 权威状态库中读取数据。
   - Q1 环境：`from plugins.nine_questions.q1_where_am_i.service import load_public_output`
   - Q2 资产：`from plugins.nine_questions.q2_asset_inventory.service import load_internal_public_output, load_external_public_output`
   - Q3 角色：`from plugins.nine_questions.q3_role_inference.service import load_public_output`
   - Q7 探索：`from plugins.nine_questions.q7_what_else_can_i_do.service import load_internal_public_output, load_external_public_output`
