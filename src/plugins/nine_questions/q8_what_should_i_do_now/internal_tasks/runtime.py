...
def _log(event: str, **fields: Any) -> None:
    observable_event(
        logger,
        event,
        component="nine_questions.q8.internal_tasks",
        question_id="q8",
        scope="internal",
        **fields,
    )
...
def _q8_internal_objective_slices(question_snapshot: dict[str, Any]) -> list[tuple[str, dict[str, Any]]]:
    q7 = _dict(question_snapshot.get("q7"))
    grouped: dict[str, list[dict[str, Any]]] = {}
    for item in _list(q7.get("internal_creative_possibilities")):
    ...
    for objective_number, possibilities in grouped.items():
        ...
        q7_slice["internal_creative_possibilities"] = possibilities
        snapshot["q7"] = q7_slice
        slices.append((objective_number, snapshot))
    return slices


def _merge_q8_string_lists(first: list[str], second: list[str]) -> list[str]:
    return merge_string_lists(first, second)
...
def _merge_q8_inference_payloads(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for result in results:
        ...
        for key in (
            ...
            objective[key] = _merge_q8_string_lists(
...
def _aggregate_q8_internal_results(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "scope": "internal",
        "raw_result": {
            "type": "Q8InternalObjectiveProfileBatch",
            "objective_results": [
                {
                    "objective_number": _text(result.get("objective_number")),
        ...
        "trace_payload": {
            "invocation_phase": "nine_question_q8_internal_decision_batch",
            "invocations": trace_invocations,
        },
        "llm_input": {
            "type": "Q8InternalObjectiveRequestBatch",
...
def run_q8_internal_task_generation(
    *,
    ...
    if not context.get("q8_internal_objective_slice_active"):
        objective_slices = _q8_internal_objective_slices(question_snapshot)
        if objective_slices:
            _log(
            ...
            for objective_number, sliced_snapshot in objective_slices:
                _log(
                    ...
                    session_id=session_id,
                    turn_id=turn_id,
                    trace_id=f"{trace_id}:internal:{objective_number}",
                    objective_number=objective_number,
                )
                slice_context = {**context, "q8_internal_objective_slice_active": True}
                result = run_q8_internal_task_generation(
            ...
            aggregate = _aggregate_q8_internal_results(objective_results)
            persist_question_module_output(
                context,
                question_id="q8",
                module_id="q8_internal_task_generation",
                payload={
                    "q8_internal_llm_input": aggregate["llm_input"],
                    "q8_internal_llm_output": aggregate["llm_output"],
                },
                status="completed",
