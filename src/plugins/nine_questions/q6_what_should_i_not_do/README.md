...
2. **官方加载器路径 (Official Loader Methods)**:
   - 必须通过上游 Q5 提供的对外读取方法从 SQLite 权威状态库中读取数据。
   - 核心方法：`from plugins.nine_questions.q5_what_am_i_allowed_to_do.service import load_public_output`，并且只消费其中的 `q5_internal_authorization_boundary` / `q5_external_authorization_boundary`，也就是 Q5 public method 已经按 `objective_number` 合并 Q4+Q5 后写入的对外结果。
