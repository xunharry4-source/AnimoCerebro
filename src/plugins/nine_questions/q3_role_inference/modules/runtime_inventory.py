...
def _log_runtime_inventory_warning(event: str, exc: Exception) -> NoReturn:
    # 严禁吞异常后伪装系统正常：Q3 运行态盘点的数据源异常必须中断 Q3。
    # 这里必须保留异常堆栈，否则稳定性问题无法定位，等同于功能假实现。
    logger.exception(
        "Q3 runtime inventory source failed: %s",
        event,
        extra={
    ...
    raise RuntimeError(f"q3_runtime_inventory_source_failed:{event}") from exc
...
def _usage_profile_map(service: Any) -> dict[str, dict[str, Any]]:
    if service is None or not callable(getattr(service, "list_usage_profiles", None)):
        return {}
    try:
        raw_profiles = service.list_usage_profiles()
    except Exception as exc:
        _log_runtime_inventory_warning("tool_usage_profile.list_usage_profiles", exc)
        raw_profiles = {}
    ...
    return {
        ...
        if (payload := _profile_to_payload(value))
    }


def _profile_verification_status(profile: dict[str, Any], *, runtime_status: str) -> str:
    normalized_runtime = _normalize_text(runtime_status)
    if not profile:
        return "unverified"
    if profile.get("learning_status") == "learned" and profile.get("degraded") is not True and normalized_runtime in {
        "active",
        "online",
    }:
        return "real_verified"
    return "degraded_learned"
...
def build_q3_runtime_inventory_context(
    ...
    *,
    ...
    context_updates = {
        f"{output_prefix}_runtime_inventory": runtime_inventory,
        f"{output_prefix}_module_results": module_results,
        f"{output_prefix}_unified_asset_inventory": unified_asset_inventory,
        f"{output_prefix}_humanized_asset_inventory": {
    ...
    module_runs = bind_module_runs(context, question_id)
...
def _derive_runtime_cli_inventory(
    ...
    if cli_service is not None and callable(getattr(cli_service, "list_tools", None)):
        try:
            ...
            raw_tools = []
    usage_profiles = _usage_profile_map(cli_service)
    ...
    for tool in raw_tools:
        ...
        profile = usage_profiles.get(tool_id, {})
        tool_rows.append(
            {
                "id": tool_id,
                "name": name,
                ...
                "status": status,
                "documentation_learning_status": _profile_verification_status(profile, runtime_status=status),
                "tool_usage_profile": profile,
                "task_routing_hints": profile.get("task_routing_hints") or [],
                "side_effects": profile.get("side_effects") or [],
    ...
    for tool in raw_tools:
        ...
        if not command_name:
            continue
        status = _normalize_text(payload.get("status")) or "active"
        profile = usage_profiles.get(command_name, {})
...
def _derive_runtime_mcp_inventory(
    ...
    if mcp_service is not None and callable(getattr(mcp_service, "list_servers", None)):
        try:
            ...
            raw_servers = []
    usage_profiles = _usage_profile_map(mcp_service)
    ...
    for server in raw_servers:
        ...
        for tool in tools:
            ...
            profile = usage_profiles.get(f"{server_id}:{tool_name}", {}) if tool_name else {}
            tool_profiles.append(
                {
                    "tool_name": tool_name,
                    "description": _normalize_text(tool.get("description")),
                    "mapped_domain": _normalize_text(tool.get("mapped_domain")),
                    "read_only": bool(tool.get("read_only", True)),
                    "mutates_state": bool(tool.get("mutates_state", False)),
                    "documentation_learning_status": _profile_verification_status(profile, runtime_status=status),
                    "tool_usage_profile": profile,
                    "task_routing_hints": profile.get("task_routing_hints") or [],
                    "side_effects": profile.get("side_effects") or [],
        ...
        server_rows.append(
            {
                ...
                "documentation_learning_status": (
                    "real_verified"
                    if tool_profiles and all(item.get("documentation_learning_status") == "real_verified" for item in tool_profiles)
                    else "unverified"
                ),
                "tool_usage_profiles": tool_profiles,
            }
        )
