...
def load_llm_output_from_table(
    *,
    ...
    if not resolved_db_path.exists():
        raise RuntimeError(f"q7_llm_output_table_missing: {resolved_db_path}")
    try:
        with sqlite3.connect(str(resolved_db_path)) as conn:
            ...
            row = conn.execute(
                ...
                FROM {Q7_SNAPSHOT_TABLE}
                WHERE session_id = ?
                """,
                (session_id,),
            ).fetchone()
    except sqlite3.OperationalError as exc:
        raise RuntimeError("q7_llm_output_table_missing") from exc
    if row is None:
        raise RuntimeError("q7_llm_output_row_missing")
    try:
        llm_output = json.loads(str(row["llm_output_json"] or "{}"))
    except json.JSONDecodeError as exc:
        raise RuntimeError("q7_llm_output_json_invalid") from exc
    if not isinstance(llm_output, dict):
        raise RuntimeError("q7_llm_output_json_not_object")
    return llm_output
