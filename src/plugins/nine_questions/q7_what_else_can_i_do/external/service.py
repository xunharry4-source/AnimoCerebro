...
def run_q7_external_llm_and_save(context: dict[str, Any]) -> dict[str, Any]:
    session_id = str(context.get("session_id") or "unknown-session")
    turn_id = str(context.get("turn_id") or context.get("request_id") or "unknown-turn")
    trace_id = f"{context.get('trace_id') or 'q7'}:external"
    request_id = f"q7-external-request:{uuid4().hex}"
    decision_id = f"q7-external:{uuid4().hex}"
    ...
    logger.info("[Q7 EXTERNAL LLM INPUT] trace_id=%s payload=%s", trace_id, json_safe_payload(llm_input))
    record_model_invoked(
        transcript_store,
    ...
    try:
        ...
        persist_question_module_output(
            ...
            payload={
                ...
                "q7_external_llm_output": llm_output,
            },
            status="completed",
        ...
        logger.exception("[Q7 EXTERNAL LLM ERROR] trace_id=%s", trace_id)
        raise
...
def _build_q7_external_upstream_context(context: dict[str, Any]) -> dict[str, Any]:
    ...
    constraint_slice = context.get("q7_q6_external_constraint_slice")
    if isinstance(constraint_slice, dict) and constraint_slice:
        upstream["Q6_ExternalPlanConstraintSet"] = constraint_slice
    else:
        upstream["Q6_ExternalPlanConstraintSet"] = load_q6_external_public_output(
