...
6. **先编号再写目标（强制映射键）**：每个候选必须先生成稳定的 `objective_number`，外部轨编号只能使用 `Q4-E-001`、`Q4-E-002`、`Q4-E-003` 这种连续格式。`objective_number` 严禁省略，严禁写成 `objective_type`、`task_id`、`subtask_id` 或真实执行工单编号。
