...
from plugins.nine_questions.q1_where_am_i.service import (
    load_workspace_domain_public_output,
)
from plugins.nine_questions.q2_asset_inventory.service import (
    load_internal_function_signal,
)
from plugins.nine_questions.q3_role_inference.service import (
    load_external_public_output as load_q3_external_public_output,
)
from plugins.nine_questions.q4_what_can_i_do.internal.llm_prompt import (
...
def run_q4_internal_llm_and_save(context: dict[str, Any]) -> dict[str, Any]:
    session_id = str(context.get("session_id") or "unknown-session")
    trace_id = f"{context.get('trace_id') or 'q4'}:internal"
    decision_id = f"q4-internal:{uuid4().hex}"
    provider = require_model_provider(context)
    require_transcript_store(context)
    prompt_context = _build_internal_prompt_context(context)
    request = build_q4_internal_llm_request(context=prompt_context)
    llm_input = {
        "prompt": request["full_prompt"],
    }
    _save_q4_internal_llm_io(session_id=session_id, llm_input=llm_input)
    logger.info("[Q4 INTERNAL LLM INPUT] trace_id=%s payload=%s", trace_id, json_safe_payload(llm_input))
    try:
        ...
        _save_q4_internal_llm_io(session_id=session_id, llm_input=llm_input, llm_output=llm_output)
        persist_question_module_output(
            context,
            question_id="q4",
            module_id="q4_internal_objective_candidate_llm",
            payload={
                "q4_internal_llm_input": llm_input,
                "q4_internal_llm_output": llm_output,
            },
            status="completed",
            output_kind="inference",
            trace_id=trace_id,
        )
        logger.info("[Q4 INTERNAL LLM OUTPUT] trace_id=%s payload=%s", trace_id, json_safe_payload(llm_output))
        return {
            "llm_input": llm_input,
        ...
        logger.exception("[Q4 INTERNAL LLM ERROR] trace_id=%s", trace_id)
        raise


def _extract_internal_result(llm_output: dict[str, Any]) -> dict[str, Any]:
    if llm_output.get("type") == "InternalObjectiveCandidateSet":
        return llm_output
    raise RuntimeError("q4_internal_objective_candidate_set_missing")


def _load_internal_reflection_gap_signal(context: dict[str, Any]) -> dict[str, Any]:
    ...
    if not callable(query):
        raise RuntimeError("q4_internal_reflection_current_problem_content_query_unavailable")

    result = query(problem_scope="internal")
    if not isinstance(result, dict):
        raise RuntimeError(
            f"q4_internal_reflection_current_problem_content_query_invalid_result:{type(result).__name__}"
        )
    return json_safe_payload(result)


def _build_internal_prompt_context(context: dict[str, Any]) -> dict[str, Any]:
    ...
    q2_internal_output = load_internal_function_signal(db_path=db_path, session_id=session_id)
    q3_external_output = load_q3_external_public_output(db_path=db_path, session_id=session_id)

    return {
        "Q3_InternalIdentityRole": q3_external_output,
        "Q1_EnvironmentObjectiveSignal_Internal": q1_output,
        "Q2_SelfObservationObjectiveSignal_Internal": q2_internal_output,
        "Reflection_CapabilityGapSignal_Internal": _load_internal_reflection_gap_signal(context),
...
def _load_q1_business_llm_output(*, db_path: Any, session_id: str) -> dict[str, Any]:
    return load_workspace_domain_public_output(db_path=db_path, session_id=session_id)


def _save_q4_internal_llm_io(
    *,
    session_id: str,
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        if row is not None:
            ...
            if isinstance(loaded, dict):
                payload = loaded
        payload["q4_internal_llm_input"] = json_safe_payload(llm_input)
        if llm_output is None:
            payload.pop("q4_internal_llm_output", None)
        else:
            payload["q4_internal_llm_output"] = json_safe_payload(llm_output)
        conn.execute(
            f"""
        ...
        if llm_output is not None:
            _upsert_q4_internal_module_output(
                conn,
                session_id=session_id,
                payload={
                    "q4_internal_llm_input": json_safe_payload(llm_input),
                    "q4_internal_llm_output": json_safe_payload(llm_output),
                },
                status="completed",
                updated_at=now,
            )
...
def _upsert_q4_internal_module_output(
    ...
    *,
    ...
    conn.execute(
        ...
        VALUES (?, 'q4', 'q4_internal_objective_candidate_llm', 1, 1, ?, 'inference', ?, ?, ?)
