...
def load_internal_q2_asset_inventory(context: dict[str, Any]) -> dict[str, Any]:
    return _normalize_q2_llm_asset_inventory(
        load_q2_internal_llm_output_from_table(db_path=context.get("nine_question_state_db_path"))
    )


def render_internal_q2_asset_inventory(context: dict[str, Any]) -> str:
    internal_inventory = load_internal_q2_asset_inventory(context)
    return (
        "Q2 内部资产清单（来自 Q2 internal_tools LLM 输出）\n"
        f"{json.dumps(json_safe_payload({'q2_internal_tool_asset_inventory': internal_inventory}), ensure_ascii=False, indent=2)}"
    )
