...
from zentex.tasks import TaskStatus, TaskType, ZentexTask
...
class AgentCoordinationService:
    ...
    def list_active_agents(self) -> List[AgentAsset]:
        ...
        return [asset for asset in self.manager.list_assets() if asset.status == AgentStatus.ACTIVE]

    def get_agent_asset_statistics(self) -> Dict[str, Any]:
        ...
        for asset in assets:
            ...
            trust_level = str(getattr(asset.trust_level, "value", asset.trust_level))
            status_counts[status] = status_counts.get(status, 0) + 1
            trust_counts[trust_level] = trust_counts.get(trust_level, 0) + 1
            capabilities = getattr(asset, "capabilities", []) or []
            service_hooks = getattr(asset, "service_hooks", []) or []
            protocol_capabilities = getattr(asset, "protocol_capabilities", []) or []
            total_capabilities += len(capabilities) + len(service_hooks) + len(protocol_capabilities)
        return {
            "asset_type": "agent",
            ...
            "blocked_agents": status_counts.get("invocation_blocked", 0),
            "trusted_agents": trust_counts.get("trusted", 0),
            "pending_agents": trust_counts.get("pending", 0),
            "revoked_agents": trust_counts.get("revoked", 0),
    ...
    async def activate_agent(
        ...
        *,
        ...
        if not self._should_probe_health(asset):
            raise ValueError(
                f"Agent {agent_id} cannot be activated because no real health probe is configured"
            )
        is_healthy = await self._adapter().check_health(
            asset,
        ...
        if not is_healthy:
            updated = self.manager.update_asset(agent_id, status=AgentStatus.OFFLINE)
            if updated:
                self._persist_agent(updated, action="activation_health_check_failed")
            raise ValueError(f"Agent {agent_id} health probe failed; activation denied")
        ...
        self._persist_agent(updated, action="activated")
        self.record_audit(agent_id, "ACTIVATED", {"health_probe": True})
        return updated

    async def disable_agent(self, agent_id: str) -> AgentAsset:
        asset = self.manager.get_asset(agent_id)
        if not asset:
            raise KeyError(f"Agent {agent_id} not found")
        updated = self.manager.update_asset(agent_id, status=AgentStatus.OFFLINE)
        if not updated:
            raise KeyError(f"Agent {agent_id} update failed")
        self._persist_agent(updated, action="disabled")
        self.record_audit(agent_id, "DISABLED", {})
        return updated
    ...
    def _should_probe_health(asset: AgentAsset) -> bool:
        ...
        if isinstance(config.get("health_probe"), dict):
            return True
        if str(getattr(asset, "adapter_type", "legacy_bridge")) in {"cli", "mcp"}:
            return True
    ...
    async def dispatch_task(
        ...
        *,
        ...
        block_reason = await self.get_dispatch_block_reason(
            agent_id,
            cli_service=cli_service,
            mcp_service=mcp_service,
        )
        if block_reason:
            return ServiceResponse.error(ServiceErrorCode.DEPENDENCY_UNAVAILABLE, block_reason)

        trace_id = _payload_string(task_payload, "trace_id") or str(uuid4())
        invocation_id = _payload_string(task_payload, "invocation_id") or f"{trace_id}:{uuid4().hex[:8]}"
    ...
    async def receive_callback_result(
        ...
        *,
        ...
        try:
            ...
            return ServiceResponse.error(ServiceErrorCode.INVALID_ARGUMENT, str(exc))
        if record is None:
    ...
    def _dispatch_block_reason(self, asset: AgentAsset) -> str:
        if asset.status != AgentStatus.ACTIVE:
            return f"Agent {asset.agent_id} is not active: {asset.status.value}"
        return ""

    async def get_dispatch_block_reason(
        self,
        agent_id: str,
        *,
        cli_service: Any = None,
        ...
        local_reason = self._dispatch_block_reason(asset)
        if local_reason:
            return local_reason
        if not self._should_probe_health(asset):
            return f"Agent {agent_id} has no real health probe configured"
        if not await self._adapter().check_health(
            asset,
            dependencies=AgentInvocationDependencies(
                cli_service=cli_service,
                mcp_service=mcp_service,
            ...
            updated = self.manager.update_asset(agent_id, status=AgentStatus.OFFLINE)
            if updated:
                self._persist_agent(updated, action="dispatch_health_check_failed")
            return f"Agent {agent_id} health probe failed"
        return ""
