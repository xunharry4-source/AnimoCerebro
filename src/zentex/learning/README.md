...
This module implements the learning engine and adaptive capabilities of the Zentex system. It includes DSPy integration, tool self-study processing, sandboxed learning environments, and budget management.

本模块实现Zentex系统的学习引擎和自适应能力。它包括DSPy集成、工具自学处理、沙盒学习环境和预算管理。
...
### Tool Self-Study Pipeline / 工具自学管道
...
from zentex.learning.tool_dspy_signatures import ToolDistillationModule
```
...
- **Tool Self-Study Pipeline** (`tool_self_study_pipeline.py`): Learning pipeline — delegates ALL input preprocessing to `llm_prompt.py` / 工具自学管道 — 所有输入预处理委托给 `llm_prompt.py`
...
- **llm_prompt.py**: **唯一负责组装 LLM / DSPy 输入和内容预处理的文件。**
  - `build_tool_distillation_inputs()` — 预处理 `doc_url` + `feedback_history`，输出直接传入 `ToolDistillationModule`。
  - `build_tool_critic_inputs()` — 预处理 critic 的四个输入字段，截断过长 schema / test_cases。
...
| `tool_self_study_pipeline.py` | 编排 Voyager 循环、调用 DSPy module、写 transcript | 禁止内联字符串截断或 prompt 拼接 |
