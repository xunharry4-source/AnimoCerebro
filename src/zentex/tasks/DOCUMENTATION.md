...
│   TaskManagementService  ──  complete_task_with_verification()   │
│      │                             └── _persist_execution_result │
│   TaskRegistry    Plugin Registry                                │
└──────────────────────────────────────────────────────────────────┘
                                  │
┌──────────────────────────────────────────────────────────────────┐
│              ★ 执行闭环层 (Execution Loop Layer)  [新增]          │
│                                                                  │
│  TaskAutoLoopScheduler  (每 15 秒一个周期)                        │
│    ├── Pass 1: TaskExecutionWorker.run_cycle()                   │
│    │     ├── 查 TODO 任务 + 检查 depends_on                       │
│    │     ├── UnifiedTaskRouter → 选最优插件                       │
│    │     ├── InternalPluginExecutor → 真实执行插件                 │
│    │     └── 写 execution_output / status / 更新信用分            │
│    ├── Pass 2: 自动恢复 suspended tasks                           │
│    └── Pass 3: 超时检测 + 重入队列                                │
└──────────────────────────────────────────────────────────────────┘
                                  │
┌──────────────────────────────────────────────────────────────────┐
│                      数据访问层 (Data Layer)                       │
│   TaskDAO (SQLite)    TaskPersistence    Transcript Store         │
│      └── migrate_task_schema()  ← 自动 ALTER TABLE 补新列         │
└──────────────────────────────────────────────────────────────────┘
                                  │
┌──────────────────────────────────────────────────────────────────┐
│                      数据模型层 (Model Layer)                      │
│   ZentexTask  (+execution_output, +dispatch_plugin_id 等 6 字段) │
│   SuspendedTask    TaskContract    SubtaskIntent                  │
└──────────────────────────────────────────────────────────────────┘
```
