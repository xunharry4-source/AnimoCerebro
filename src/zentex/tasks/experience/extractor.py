...
class ExperienceExtractor:
    ...
    async def extract_experience_context(
        ...
        try:
            ...
            logger.error(f"Failed to extract similar tasks: {e}")
        ...
        try:
            ...
            logger.error(f"Failed to extract executor performance: {e}")
            ...
            for result in search_results:
                ...
                if similarity >= self.similarity_threshold:
                    ...
                    record = ExperienceRecord(
                        ...
                        task_type=task_type,
                        executor_id=result.get("target_id", "unknown"),
    ...
    async def extract_lessons_from_failures(
        ...
        if len(timeout_tasks) >= 2:
            avg_time = mean([t.completion_time_seconds or 0 for t in timeout_tasks if t.completion_time_seconds])
            lessons.append(
                LessonLearned(
        ...
        if len(successful_tasks) >= 2 and successful_tasks:
            avg_time = mean([t.completion_time_seconds or 300 for t in successful_tasks if t.completion_time_seconds])
            lessons.append(
                LessonLearned(
                    category=LessonCategory.BEST_PRACTICE,
                    title=f"Proven success pattern",
                    content=f"Similar {task_type} tasks typically succeed within {int(avg_time)}s when well-structured.",
                    confidence=ConfidenceLevel.MEDIUM,
    ...
    def _infer_outcome(self, memory_record: Dict[str, Any]) -> TaskOutcomeType:
        """Infer outcome from memory record."""
        content = memory_record.get("content", "").lower() + memory_record.get("summary", "").lower()
        
        if "timeout" in content or "timed out" in content:
            return TaskOutcomeType.TIMEOUT
        elif "failed" in content or "failure" in content or "error" in content:
            return TaskOutcomeType.FAILURE
        elif "partial" in content or "incomplete" in content:
            return TaskOutcomeType.PARTIAL_SUCCESS
        elif "success" in content or "completed" in content or "done" in content:
