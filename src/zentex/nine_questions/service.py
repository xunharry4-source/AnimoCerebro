...
class NineQuestionService:
    ...
    @staticmethod
    def _is_q5_dual_lane_llm_snapshot(snapshot: dict[str, Any]) -> bool:
        llm_output = snapshot.get("llm_output")
        if not isinstance(llm_output, dict):
            return False
        return (
            isinstance(llm_output.get("q5_internal_llm_input"), dict)
            and bool(llm_output.get("q5_internal_llm_input"))
            and isinstance(llm_output.get("q5_internal_llm_output"), dict)
            and bool(llm_output.get("q5_internal_llm_output"))
            and isinstance(llm_output.get("q5_external_llm_input"), dict)
            and bool(llm_output.get("q5_external_llm_input"))
            and isinstance(llm_output.get("q5_external_llm_output"), dict)
            and bool(llm_output.get("q5_external_llm_output"))
        )

    @staticmethod
    def _is_q6_dual_lane_llm_snapshot(snapshot: dict[str, Any]) -> bool:
        llm_output = snapshot.get("llm_output")
        if not isinstance(llm_output, dict):
            return False
        return (
            isinstance(llm_output.get("q6_internal_llm_input"), dict)
            and bool(llm_output.get("q6_internal_llm_input"))
            and isinstance(llm_output.get("q6_internal_llm_output"), dict)
            and bool(llm_output.get("q6_internal_llm_output"))
            and isinstance(llm_output.get("q6_external_llm_input"), dict)
            and bool(llm_output.get("q6_external_llm_input"))
            and isinstance(llm_output.get("q6_external_llm_output"), dict)
            and bool(llm_output.get("q6_external_llm_output"))
        )
    ...
    def _validate_public_service_upstreams(self, question_id: str) -> list[str]:
        qid = str(question_id or "").strip().lower()
        if qid not in {"q3", "q4", "q8", "q9"}:
            return []
        ...
        if qid in {"q3", "q4", "q8"}:
            try:
                ...
                load_q2_external_public_output(db_path=db_path)
                if qid in {"q4", "q8"}:
                    load_q2_internal_public_output(db_path=db_path)
            except Exception as exc:
                failures.append(f"q2:public_service_output_not_qualified:{exc}")
        ...
        if qid == "q8":
            try:
                ...
                load_q3_public_output(db_path=db_path)
            except Exception as exc:
                failures.append(f"q3:public_service_output_not_qualified:{exc}")
        if qid == "q9":
            try:
                ...
                load_q8_internal_public_output(db_path=db_path)
                load_q8_external_public_output(db_path=db_path)
            except Exception as exc:
                failures.append(f"q8:public_service_output_not_qualified:{exc}")
        return failures
    ...
    async def assert_latest_qualified_upstreams(self, question_id: str) -> dict[str, Any]:
        ...
        Snapshot-backed upstreams must be latest-qualified. Public-service
        upstreams are validated only through upstream-owned service accessors.
        """
        qid = str(question_id or "").strip().lower()
        required = self._snapshot_gate_upstream_questions(qid)
        public_service_failures = self._validate_public_service_upstreams(qid)
        if not required and not public_service_failures:
        ...
        failures: list[str] = list(public_service_failures)
    ...
    async def persist_question_snapshot_patch(
        ...
        *,
        ...
        elif question_id == "q5" and self._is_q5_dual_lane_llm_snapshot(patch):
            decision = {
                "accept_snapshot": True,
                "merge_diagnostics_only": False,
                "preserve_previous_success": False,
                "reason": "q5_dual_lane_llm_snapshot_accepted",
            }
        elif question_id == "q6" and self._is_q6_dual_lane_llm_snapshot(patch):
            decision = {
                "accept_snapshot": True,
                "merge_diagnostics_only": False,
                "preserve_previous_success": False,
                "reason": "q6_dual_lane_llm_snapshot_accepted",
            }
        else:
