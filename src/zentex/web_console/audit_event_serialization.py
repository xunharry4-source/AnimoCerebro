...
def extract_context_info(entry: _AuditEntryLike) -> Dict[str, Any]:
...
def serialize_audit_event(
    entry: _AuditEntryLike,
    *,
    include_payload: bool = True,
) -> AuditEventPayload:
    raw_timestamp = entry.timestamp
    if isinstance(raw_timestamp, datetime):
        serialized_timestamp = raw_timestamp.astimezone(timezone.utc).isoformat()
    elif isinstance(raw_timestamp, str):
        serialized_timestamp = raw_timestamp
    else:
        serialized_timestamp = str(raw_timestamp)
