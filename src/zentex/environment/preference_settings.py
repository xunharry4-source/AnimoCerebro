...
class AlignmentSettings(BaseSettings):
    ...
    所有配置项可通过环境变量覆盖，前缀为 Alignment_
    例如：Alignment_AUTO_CONFIRM_THRESHOLD=0.95
    """
    
    model_config = SettingsConfigDict(
        env_prefix="Alignment_",
        env_file=".env",
    ...
    sqlite_db_path: Path = Field(
        default_factory=lambda: get_storage_paths().app_data_dir / "preference_preference_store.db",
...
# 全局单例
_settings: Optional[AlignmentSettings] = None


def get_alignment_settings() -> AlignmentSettings:
    """
    获取偏好对齐配置单例
    
    Returns:
        AlignmentSettings: 配置对象
...
def reload_alignment_settings() -> AlignmentSettings:
    """
    重新加载配置（用于热重载）
    
    Returns:
        AlignmentSettings: 新的配置对象
        
    Example:
        >>> # 修改环境变量后重新加载
        >>> import os
        >>> os.environ['ALIGNMENT_AUTO_CONFIRM_THRESHOLD'] = '0.95'
        >>> settings = reload_alignment_settings()
        >>> print(settings.auto_confirm_threshold)
        0.95
    """
    global _settings
...
def get_preference_settings() -> AlignmentSettings:
    """Backward-compatible alias for legacy imports."""
    return get_alignment_settings()
