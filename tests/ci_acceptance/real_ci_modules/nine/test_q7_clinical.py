...
@pytest.mark.asyncio
async def test_q7_clinical_authenticity():
    """
    Q7 临床验收测试：验证认知真实性与服务层完整性。
    ...
    # 3. 获取问题定义
    q = next(q for q in DEFAULT_NINE_QUESTIONS if q.question_id == "q7")
    trace_id = "ci-acceptance-trace-q7"

    # 3.1 严格门禁：只允许读取已存在的上游最新合格快照，不在 Q7 测试中补跑上游题目
    try:
        await nq_query_service.assert_latest_qualified_upstreams("q7")
    except Exception as exc:
        raise AssertionError(
            f"Q7 prerequisite gate failed. Upstreams must already be latest-qualified snapshots. {exc}"
        ) from exc
    ...
    print(f"[Clinical] Q7 Authenticity Status: {authenticity}")
    ...
    assert isinstance(module_runs, list) and len(module_runs) > 0, "No module_runs found in q7 execution diagnosis"
    ...
    assert not missing_run_modules, f"Missing required q7 module runs: {sorted(missing_run_modules)}"

    modules_payload = await nq_query_service.get_question_modules(q.question_id)
    module_map = modules_payload.get("modules", {})
    assert isinstance(module_map, dict) and module_map, "No persisted module payload for q7"
    ...
    assert any("q7" in str(item).lower() or trace_id in str(item) for item in audit_flows), (
        "No Q7-related audit flow found"
    )
    ...
    assert any(
        "q7" in str(item).lower()
        or trace_id in str(item)
        or "替代策略" in str(item)
        or "alternative_strategy" in str(item).lower()
        for item in learning_entries
    ), (
        "No Q7-related learning entry found"
    )

if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    asyncio.run(test_q7_clinical_authenticity())
