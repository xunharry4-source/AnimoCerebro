...
def _build_question_parameters(question_id: str, question_text: str) -> dict:
    return {
        ...
        "llm_model": CI_ACCEPTANCE_OLLAMA_MODEL,
        "model": CI_ACCEPTANCE_OLLAMA_MODEL,
        "request_timeout_seconds": CI_ACCEPTANCE_REQUEST_TIMEOUT_SECONDS,
    }
...
def _q8_log(message: str) -> None:
    print(f"[Clinical][Q8] {message}", flush=True)


@contextmanager
def _q8_timed_step(label: str):
    started = perf_counter()
    _q8_log(f"START {label}")
    try:
        yield
    finally:
        elapsed = perf_counter() - started
        _q8_log(f"END {label}: {elapsed:.3f}s")
...
def _execute_with_retry(
    *,
    ...
    for attempt in range(1, max_attempts + 1):
        with _q8_timed_step(f"plugin execution attempt {attempt}/{max_attempts}"):
        ...
        _q8_log(
            ...
            f"remarks={getattr(response, 'remarks', '')!r}"
        )
...
async def test_q8_clinical_comprehensive():
    ...
    # 3. 注入依赖到内核
    with _q8_timed_step("attach dependencies and register plugins"):
    ...
    with _q8_timed_step("assert latest qualified upstream snapshots q1-q7"):
        try:
    ...
    # 5. 验证数据库与权威状态 (Q8)
    with _q8_timed_step("read authoritative nq-baseline state"):
    ...
    with _q8_timed_step("build authoritative q8 question record"):
    ...
    with _q8_timed_step("load q8 persisted module map"):
    ...
    with _q8_timed_step("list tasks for q8 synchronization check"):
    ...
    with _q8_timed_step("query q8 learning entries"):
