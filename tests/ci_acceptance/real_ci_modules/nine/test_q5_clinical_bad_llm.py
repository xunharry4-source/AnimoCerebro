...
@pytest.mark.asyncio
async def test_q5_clinical_authenticity():
    """
    Q5 临床验收测试：验证认知真实性与服务层完整性。
    ...
    # 3. 获取问题定义
    q = next(q for q in DEFAULT_NINE_QUESTIONS if q.question_id == "q5")
    trace_id = "ci-acceptance-trace-q5"

    # 3.1 严格门禁：只允许读取已存在的上游最新合格快照，不在 Q5 测试中补跑上游题目
    try:
        await nq_query_service.assert_latest_qualified_upstreams("q5")
    except Exception as exc:
        raise AssertionError(
            f"Q5 prerequisite gate failed. Upstreams must already be latest-qualified snapshots. {exc}"
        ) from exc
    ...
    print(f"[Clinical] Q5 Authenticity Status: {authenticity}")
    ...
    assert isinstance(module_runs, list) and len(module_runs) > 0, "No module_runs found in q5 execution diagnosis"
    ...
    assert not missing_run_modules, f"Missing required q5 module runs: {sorted(missing_run_modules)}"

    modules_payload = await nq_query_service.get_question_modules(q.question_id)
    module_map = modules_payload.get("modules", {})
    assert isinstance(module_map, dict) and module_map, "No persisted module payload for q5"
    ...
    required_persisted_modules = {
        ...
        "q5_agent_trust_validation",
        "q5_authorization_decision_projection",
        "q5_memory_integration",
        "q5_reflection_integration",
    }
    ...
    assert any(
        "q5" in str(getattr(item, "title", "")).lower()
        or "q5" in str(getattr(item, "summary", "")).lower()
        or "q5" in " ".join(getattr(item, "tags", []) or []).lower()
        for item in memory_records
    ), "No Q5-related memory record found for trace_id"
    ...
    assert any(
        ...
        or "q5" in str(getattr(item, "subject", "")).lower()
        or "q5" in str((getattr(item, "context", {}) or {}).get("question_id", "")).lower()
        for item in reflection_records
    ), "No Q5-related reflection record found for trace_id"
    ...
    assert any("q5" in str(item).lower() or trace_id in str(item) for item in audit_flows), (
        "No Q5-related audit flow found"
    )
    ...
    learning_found = any("q5" in str(item).lower() or trace_id in str(item) for item in learning_entries)
    if not learning_found:
        learning_module = module_map.get("q5_learning_integration", {}) if isinstance(module_map, dict) else {}
        ...
        assert learning_trace_id, "No Q5-related learning entry found"

if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    asyncio.run(test_q5_clinical_authenticity())
