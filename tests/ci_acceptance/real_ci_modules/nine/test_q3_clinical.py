...
@pytest.mark.asyncio
async def test_q3_clinical_authenticity():
    """
    Q3 临床验收测试：验证认知真实性与服务层完整性。
    ...
    # 3. 获取问题定义
    q = next(q for q in DEFAULT_NINE_QUESTIONS if q.question_id == "q3")
    trace_id = "ci-acceptance-trace-q3"
    ...
    assert getattr(response, "status", None) in ("completed", "done"), (
    ...
    print(f"[Clinical] Q3 Authenticity Status: {authenticity}")
    ...
    assert not missing_run_modules, f"Missing required q3 module runs: {sorted(missing_run_modules)}"

    modules_payload = await nq_query_service.get_question_modules(q.question_id)
    module_map = modules_payload.get("modules", {})
    assert isinstance(module_map, dict) and module_map, "No persisted module payload for q3"
    ...
    assert any(
        "q3" in str(item).lower() or trace_id in str(item)
        for item in audit_flows
    ), "No Q3-related audit flow found"
    ...
    if not learning_found:
        learning_module = module_map.get("q3_learning_integration", {}) if isinstance(module_map, dict) else {}
        ...
        assert learning_trace_id, "No Q3-related learning entry found"

if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    asyncio.run(test_q3_clinical_authenticity())
