...
async def test_task_complete_real(real_ci_runtime) -> None:
    ...
    suffix = uuid4().hex[:10]
    source_module = f"ci_task_complete_{suffix}"
    ...
    try:
        ...
        assert [item.task_id for item in grouped_before["pending"]] == [task.task_id], "TODO 任务应在 pending 分组"
        ...
        assert [item.task_id for item in grouped_mid["in_progress"]] == [task.task_id], (
            "in_progress 任务应在 in_progress 分组"
        )
        ...
        assert [item.task_id for item in grouped_final["completed"]] == [task.task_id], "done 任务应在 completed 分组"
    finally:
