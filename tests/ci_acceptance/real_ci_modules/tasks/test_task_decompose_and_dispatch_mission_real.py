...
async def test_task_decompose_and_dispatch_mission_real(real_ci_runtime) -> None:
    ...
    # 查询校验3：按 parent_task_id 列表查询可命中全部子任务
    listed = real_ci_runtime.task_service.list_tasks(parent_task_id=mission.task_id)
    listed_ids = {item.task_id for item in listed}
    assert set(queried_mission.subtask_ids).issubset(listed_ids), "按父任务查询未覆盖全部子任务"
