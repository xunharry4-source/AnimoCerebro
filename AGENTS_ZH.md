...
**🔄 注意**：九问框架已经历重大重构（v2.1.0-alpha）。以下描述反映的是**新架构**，具有增强的模块化和双重规划系统。

1. **Q1 我在哪？**（环境感知）- 增强的证据收集，具有更好的验证和清理机制
2. **Q2 我是谁？**（角色推断）- 语义分析改进进行中（完成度 40%）
3. **Q3 我有什么？**（运行时库存）- **重大重构**：拆分为内部/外部任务模块，带有专用的规划器、验证器和上下文构建器
4. **Q4 我能做什么？**（能力评估）- 动态能力注册，带 instructor contracts（完成度 35%）
5. **Q5 我被允许做什么？**（授权边界）- 边界投影和约束分析增强（完成度 20%）
6. **Q6 我不应该做什么？**（风险预防）- 增强的风险识别，改进的输出契约（完成度 20%）
7. **Q7 我还能做什么？**（替代方案探索）- 用于替代方案评估的 assessment contracts（完成度 20%）
8. **Q8 我现在应该做什么？**（任务规划）- **重大重构**：双重规划架构，将内部任务（核心操作）与外部任务（第三方连接器）分离
9. **Q9 我应该如何行动？**（行动执行）- 增强的行动计划，具有更好的上下文感知和独立的执行路径

**关键架构改进**：
- ✅ Q3、Q8、Q9 分离了内部/外部处理
- ✅ 所有问题都使用 instructor contracts 进行结构化输出
- ✅ 增强的提示模板和输出契约
- ✅ 更好的模块化和减少冗余
- ✅ 提高主动性和灵活性
